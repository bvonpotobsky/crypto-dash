...
const CurrencyMetrics: React.FC<{currency: CurrencyByIdType}> = ({currency}): JSX.Element => {
  return (
    <div className="w-full col-start-1 col-end-3 lg:col-end-2 flex flex-col rounded overflow-hidden">
      ...
      <ul className="w-full flex flex-col items-start justify-between bg-white dark:bg-slate-800/50 [&>*]:w-full [&>*]:p-2 lg:[&>*]:p-2.5">
        <li className="flex items-center justify-between">
          Price <span className="ml-auto">{formatPrice(currency?.market_data?.current_price?.usd)} USD</span>
        </li>
        <li className="flex items-center justify-between">
          Real Volume (24H)
